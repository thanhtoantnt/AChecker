...
def get_contract_names(filename: str):
    log_lines = []
    try:
        ...
        print("Cannot read the input file: ", filename)
        exit()

    regex = (r"contract ([a-zA-Z$_][a-zA-Z0-9$_]*)")
    contracts = []
    for log_line in log_lines:
        if match := re.search(regex, log_line):
...
def get_evm(contract):
    ...
    path = os.path.abspath(contract)
    path_dir = os.path.dirname(path)
    cmd = "solc --bin-runtime %s" % path
    print(cmd)
    FNULL = open(os.devnull, "w")
...
def analysis(
    ...
    user_alerts = {
        "Violated-AC-Check": "Violated access control check",
    ...
    for defect_type in list(["Violated-AC-Check", "Missing-AC-Check"]):
        ...
        if defect_type in (["Violated-AC-Check"]):
            ...
            (
                ac_checks_with_sloads,
                slices,
                imap,
                other_ac_cehcks,
            ) = p.resolve_access_control_slots(
                ssa,
                ins,
                ac_check_ins=opcodes.ins_in_ac_check,
                args=args,
            ...
            for k in ac_checks_with_sloads:
                ac_checks_sloads.extend(
                    [
                        ins
                        for ins in [s["sload"] for s in ac_checks_with_sloads[k]]
                        if ins in storage_info
                    ]
                )
            ac_sload_slots = list(
                set(
                    [
                        s
                        for ins in ac_checks_sloads
                        if ins in storage_info
                        for s in storage_info[ins].reads
                    ]
                )
            )
            ac_sload_sha3_bases = {
                s: storage_info[ins].read_sha3_bases[s]
                for ins in ac_checks_sloads
                if ins in storage_info
                for s in storage_info[ins].reads
                if storage_info[ins].read_sha3_bases
            }
            ...
            s_temp = [
                ...
                for ins in sstores
                if ins in storage_info
                and storage_info[ins].writes.union(
                    set(storage_info[ins].write_sha3_bases.values())
                )
                & set(ac_sload_slots + list(ac_sload_sha3_bases.values()))
            ]
            sstores = s_temp
            ...
            for jump_ins in ac_checks_with_sloads:
                current_ac_check_slots = [
                    s
                    for ins in [s["sload"] for s in ac_checks_with_sloads[jump_ins]]
                    if ins in storage_info
                    for s in storage_info[ins].reads
                ]
                current_ac_check_slot_sha3_bases = [
                    ac_sload_sha3_bases[slot]
                    for slot in current_ac_check_slots
                    if len(str(slot)) > 5 and slot in ac_sload_sha3_bases
                ]
                current_ac_check_slot_sbyte = [
                    {"slot": s, "sbyte": sload["sbyte"]}
                    for s in current_ac_check_slots
                    if len(str(s)) < 5
                    for sload in ac_checks_with_sloads[jump_ins]
                ]
                current_ac_check_struct_offset = [
                    sload["structOffset"]
                    for s in current_ac_check_slots
                    for sload in ac_checks_with_sloads[jump_ins]
                ]
                current_ac_check_slots_sstores = [
                    ins
                    for ins in storage_info
                    if ins.name == "SSTORE"
                    and storage_info[ins].writes & set(current_ac_check_slots)
                ]
                for sstore_ins in tainted_sstores:
                    current_sstore_slot = [s for s in storage_info[sstore_ins].writes]
                    if set(current_sstore_slot) & set(
                        [s["slot"] for s in current_ac_check_slot_sbyte]
                    ):
                        if not set([sstore_slot_sbyte[sstore_ins]]).union(
                            {"whole"}
                        ) & set([s["sbyte"] for s in current_ac_check_slot_sbyte]):
                            tainted_sstores_tmp.append(sstore_ins)
                    current_sstore_slot_sha3_base = [
                        storage_info[sstore_ins].write_sha3_bases[s]
                        for s in current_sstore_slot
                        if len(str(s)) > 5
                        and s in storage_info[sstore_ins].write_sha3_bases
                    ]
                    if set(current_sstore_slot) & set(
                        [s["slot"] for s in current_ac_check_slot_sbyte]
                    ) or set(current_ac_check_slot_sha3_bases) & set(
                        current_sstore_slot_sha3_base
                    ):
                        if (
                            sstore_struct_offset[sstore_ins]
                            not in current_ac_check_struct_offset
                        ):
                            tainted_sstores_tmp.append(sstore_ins)
            ...
            while todo or (progress and resolve_later):
                ...
                logging.debug(
                    "Path: %s", "->".join("%x" % p for p in tainted_sstores[sstore_ins])
                )
                sstore_path_ac_checks = [
                    ins
                    for ins in storage_info
                    if ins in ac_checks_sloads
                    and ins.bb.start in tainted_sstores[sstore_ins][:-1]
                ]
                sstore_path_other_ac_checks = [
                    ins
                    for ins in other_ac_cehcks
                    if ins.bb.start in tainted_sstores[sstore_ins][:-1]
                ]

                if (
                    len(sstore_path_ac_checks) == 0
                    and len(sstore_path_other_ac_checks) == 0
                ):
                    if sym_validation:
                        try:
                            taint_valid_results = validate_path(
                                p, tainted_sstores[sstore_ins]
                            )
                    ...
                    other_access_sanitizers = [
                        slot
                        for ins in sstore_path_ac_checks
                        for slot in storage_info[ins].reads
                    ]
                    other_access_sanitizers_sstores = [
                        ins
                        for ins in storage_info
                        if ins.name == "SSTORE"
                        and storage_info[ins].writes & set(other_access_sanitizers)
                    ]
                    if set(other_access_sanitizers_sstores) & set(
                        checked_tainted_sstores
                    ):
                        if sym_validation:
                            taint_valid_results = validate_path(
                    ...
                    if set(other_access_sanitizers_sstores) & set(not_tainted_sstore):
                        not_tainted_sstore.append(sstore_ins)
                        continue
                    if tainted_sstores.keys() & set(
                        other_access_sanitizers_sstores
                    ) != set(other_access_sanitizers_sstores):
                        not_tainted_sstore.append(sstore_ins)
                        continue
                    if tainted_sstores.keys() & set(other_access_sanitizers_sstores):
                        sstore_fun_sig = cinfo.get_function_sig(
                            p.cfg, tainted_sstores[sstore_ins], "id"
                        )
                        other_tainted_santizers_sstores = [
                            ins
                            for ins in tainted_sstores.keys()
                            if set([ins]) & set(other_access_sanitizers_sstores)
                            and sstore_fun_sig
                            != cinfo.get_function_sig(p.cfg, tainted_sstores[ins], "id")
                        ]
                        if other_tainted_santizers_sstores and set(
                            other_tainted_santizers_sstores
                        ) & set(checked_tainted_sstores) == set(
                            other_tainted_santizers_sstores
                        ):
                            if sym_validation:
                                taint_valid_results = validate_path(
            ...
            for i, i_path in p.extract_control_paths(slices, imap):
                ...
                current_ac_check_slots = [
                    s
                    for ins in [s["sload"] for s in ac_checks_with_sloads[i]]
                    if ins in storage_info
                    for s in storage_info[ins].reads
                ]
                current_ac_check_slot_sbyte = [
                    ...
                    for s in current_ac_check_slots
                    if len(str(s)) < 5
                    for sload in ac_checks_with_sloads[i]
                ]
                current_ac_check_slots_sha3_bs = [
                    ac_sload_sha3_bases[slot]
                    for slot in current_ac_check_slots
                    if len(str(slot)) > 5 and slot in ac_sload_sha3_bases
                ]
                current_ac_check_slots_sstores = [
                    ...
                    if ins.name == "SSTORE"
                    and storage_info[ins].writes & set(current_ac_check_slots)
                    or set(storage_info[ins].write_sha3_bases.values())
                    & set(current_ac_check_slots_sha3_bs)
                ]
                ...
                for sstore_ins in current_ac_check_slots_sstores:
                    if sstore_ins in checked_tainted_sstores:
                        current_sstore_slot = [
                            s for s in storage_info[sstore_ins].writes
                        ]
                        current_sstore_slot_sha3_bs = [
                            storage_info[sstore_ins].write_sha3_bases[s]
                            for s in storage_info[sstore_ins].writes
                            if len(str(s)) > 5
                        ]
                        sstore_slot = list(
                            set(current_sstore_slot)
                            if len(str(current_sstore_slot[0])) <= 5
                            else set(current_sstore_slot_sha3_bs)
                        )
                        sstore_in_function = cinfo.get_function_sig(
                            p.cfg, tainted_sstores[sstore_ins]
                        )
                        if (
                            ac_function
                            + "-"
                            ...
                            + "-"
                            + str(sstore_slot[0])
                            in checked_access_paths[sstore_slot[0]]
                        ):
                            continue
                        checked_access_paths[sstore_slot[0]].append(
                            ac_function
                            + "-"
                            + sstore_in_function
                            + "-"
                        ...
                        logging.debug(
                            "Path: %s",
                            "->".join("%x" % p for p in tainted_sstores[sstore_ins]),
                        )
                        if not reported:
                            print(
                                "\n{0} in function {1}".format(
                                    user_alerts[defect_type], ac_function
                                )
                            )
                            print("\t%s" % i)
                            reported = True
                        if mode == "FIntendedB":
                            ...
                            if len(ac_checks_in_sstore_path) == 0:
                                taint_valid_results = validate_path(
                                    p,
                                    tainted_sstores[sstore_ins],
                                    mode=mode,
                                    ac_jumpi=[s.addr for s in ac_checks_with_sloads],
                                )
                            if taint_valid_results:
                                ...
                                if pib:
                                    ...
                                    IntendedB = True
                                    print(
                                        "+--(Potentially Intended Behavior) Attacker can make changes to AC item {0} in function {1}".format(
                                            set(sstore_slot), sstore_in_function
                                        )
                                    )
                                    continue
                        print(
                            "+--Attacker can make changes to AC item {0} in function {1}".format(
                                set(sstore_slot), sstore_in_function
                            )
                        )
            ...
            for ins_type in ins_types:
                ...
                for (
                    ...
                    if ins_type in (["SELFDESTRUCT", "DELEGATECALL"]) and j_r._tainted:
                        ...
                        if any(
                            k
                            for i in j_r.sources
                            for k, v in i.items()
                            if k.split("-", 1)[0] != "SLOAD"
                        ):
                            ins_path_ac_checks = [
                                ins
                                for ins in storage_info
                                if ins in ac_checks_sloads
                                and ins.bb.start in j_path[:-1]
                            ]
                            ...
                            sload_sha3_bases = {
                                i: j_r.sload_sha3_bases[i]
                                for i in j_r.sload_sha3_bases
                                if i in sload_slots
                            }
                            sstores = p.cfg.filter_ins("SSTORE", reachable=True)
                            sstore_sinks = {s.addr: [1] for s in sstores}
                            sstore_taintedBy = (
                                opcodes.potentially_direct_user_controlled
                            )
                            for (
                                js,
                ...
                for s in tainted_ins:
                    logging.debug("%s", s)
                    logging.debug(
                        "Path: %s", "->".join("%x" % p for p in tainted_ins[s]["path"])
                    )
                    ac_function = cinfo.get_function_sig(
                        p.cfg, tainted_ins[s]["path"], "id"
                    )
                    ...
                    if tainting_loc[s]:
                        ...
                        ins_path_ac_checks = [
                            ...
                            for ins in storage_info
                            if ins in ac_checks_sloads
                            and ins.bb.start in tainting_loc[s][:-1]
                        ]
                        ins_path_other_ac_checks = [
                            ins
                            for ins in other_ac_cehcks
                            if ins.bb.start in tainting_loc[s][:-1]
                        ]
                        ...
                        ins_path_ac_checks = [
                            ...
                            for ins in storage_info
                            if ins in ac_checks_sloads
                            and ins.bb.start in tainted_ins[s]["path"][:-1]
                        ]
                        ins_path_other_ac_checks = [
                            ins
                            for ins in other_ac_cehcks
                            if ins.bb.start in tainted_ins[s]["path"][:-1]
                        ]
                        tainting_path = tainted_ins[s]["path"]

                    if (
                        len(ins_path_ac_checks) == 0
                        and len(ins_path_other_ac_checks) == 0
                    ):
                        checked_ins_paths[s.addr].append(
                            ac_function + "-" + str(s.addr)
                        )
                        logging.debug("%s", s)
                        logging.debug(
                            "Path: %s",
                            "->".join("%x" % p for p in tainted_ins[s]["path"]),
                        )
                        logging.debug("Path: %s", "->".join("%x" % p for p in j_path))
                        print(
                            "({0}) {1} in function {2}".format(
                                user_alerts[tainted_ins[s]["type"]],
                                user_alerts[defect_type],
                                cinfo.get_function_sig(p.cfg, tainting_path),
                            )
                        )
                        print(
                            "Needed to protect following instruction in function {0}".format(
...
def main():
    ...
    parser.add_argument(
        "-b",
        "--bytecode",
        help="read EVM bytecode in source instead of solidity file.",
        action="store_true",
    )
    ...
    if args.file is None:
        print(
            "Usage: %s  <-f file>   [--memory] [--savefile]" % sys.argv[0],
            file=sys.stderr,
        )
        exit(-1)
